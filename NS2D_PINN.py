...
# u=np.sin(x1)*np.sin(x2)+np.cos(x2)
# x_test=x1.flatten
# y_test=y.flatten
# u_test=u.flatten
# p_test=u_test
...
data = scipy.io.loadmat('NS2D_InitialData.mat')
...
# '''
pinn = NS2D(x_train, y_train, t_train, u_train, v_train)

pinn.train()

torch.save(pinn.net.state_dict(), 'ns_model.pt')
# '''

# pinn = NavierStokes(x_train, y_train, t_train, u_train, v_train)
# pinn.net.load_state_dict(torch.load('ns_model.pt'))
# pinn.net.eval()
